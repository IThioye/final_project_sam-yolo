...
try:
    sam = sam_model_registry["vit_h"](checkpoint=app.config['SAM_CHECKPOINT'])
    ...
    try:
        ...
        for point in void_points:
            ...
            if len(masks) > 0:  # Check if any masks were generated
                ...
                logger.debug(f"Processed void point {point} with score {scores[best_mask_idx]}")
        ...
        if component_boxes:
            for box in component_boxes:
                ...
                if len(masks) > 0:
                    ...
                    logger.debug(f"Processed component box {box}")
...
def classify():
    ...
    try:
        ...
        return jsonify({
            'status': 'success',
            'result_path': url_for('static', filename=f'yolo_results/{output_filename}'),
            'area_data': area_data,
            'area_data_path': url_for('static', filename=f'data/{area_data_filename}')
        })
    except Exception as e:
